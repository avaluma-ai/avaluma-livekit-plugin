    ...
    def __init__(
        ...
        *,
        ...
        if self._check_resources:
            logger.info(
                f"Initializing resource monitor for avatar_id='{self._avatar_id}'"
            )
            ...
            logger.info(
                f"Resource monitoring is disabled for avatar_id='{self._avatar_id}' (check_resources=False)"
            )
    ...
    async def _start_local(self, agent_session: AgentSession, room: rtc.Room) -> None:
        ...
        if self._check_resources and self._resource_monitor:
            ...
            try:
                ...
                result_msg = (
                    f"Resource check passed: RAM={status.available_ram_gb:.2f}GB"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not set participant attributes: {attr_error}"
                    )
